JDCOIN Core version *v4.0.0* is now available from:  <https://github.com/jdcoin-project/jdcoin/releases>
...
JDCOIN Core v4.0.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
### V1 zJDC Spending (Public Spends Version 4)

Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 zJDC have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 zJDC with public spends version 4 ([PR #936](https://github.com/JDCOIN-Project/JDCOIN/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 zJDC), version 4 spends will also be public. A full technical writeup is available on the [JDCOIN Wiki](https://github.com/JDCOIN-Project/JDCOIN/wiki/CoinRandomnessSchnorrSignature).
...
- #936 `12a6b704b6` [zJDC] PublicCoinSpend v4 - Coin Randomness Schnorr Signature (random-zebra)
...
- #1129 `a87bfc32a0` [Consensus] Define TestNet changeover block for JDCOIN v4.0 RC (random-zebra)
- #1134 `a4ded20de4` [Trivial] Remove a duplicate variable definition (warrows)
- #1191 `9a054eeba6` [Consensus] Define MainNet changeover block for JDCOIN v4.0 (random-zebra)
...
- #1125 `c6e238ca4d` [GUI] Inform if open jdcoin.conf and/or backups folder fail. (furszy)
...
- #1179 `7a33fe10d2` [Qt] Fix for dead link to wrong JDCOIN website (NoobieDev12)
